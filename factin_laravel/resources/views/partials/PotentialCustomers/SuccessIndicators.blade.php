...
@section('ScriptZone')
	<script>
		...
		$('select[name=yearSelected]').change(function (e) { 
			e.preventDefault();
			$('.spinner-Cubikom').fadeIn();
            $('.myCanvas').fadeOut();
			if (statusChart) {
				statusChart.destroy();
			}
			let year = $('select[name=yearSelected]').val();
			$.get("{{route('getLeadGraph')}}",
				function (objectLeadGraphs) {
					console.log(objectLeadGraphs);
					...
					for (const key in Mes) {					
						...
						for (const iterator of objectLeadGraphs) {
							let datayear = iterator['lead_Date'];
							let status = iterator['lead_status'];
			...
			$('.spinner-Cubikom').fadeOut(2000);
		});

		let year = $('select[name=yearSelected]').val();
		$.get("{{route('getLeadGraph')}}", 
			function (objectLeadGraph) {				
				let Mes = {'Enero': 0,'Febrero': 0,'Marzo': 0,'Abril': 0,'Mayo': 0,'Junio': 0,'Julio': 0,'Agosto': 0,'Septiembre': 0,'Octubre': 0,'Noviembre': 0,'Diciembre': 0};
				let Approved = [], NonApproved = [], countApproved = 0, countNonApproved = 0;
				for (const key in Mes) {					
					countApproved = 0, countNonApproved = 0;
					for (const iterator of objectLeadGraph) {
						let datayear = iterator['lead_Date'];
						let status = iterator['lead_status'];
						let years = datayear.split("-");
						let myYear = years[2], myMonth = years[1];
